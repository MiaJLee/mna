...
export default function GreetingSection({ config }: { config: WeddingConfig }) {
	return (
				...
				<div className="text-center py-8">
					...
					<div className="mt-8 text-sm text-warm-gray">
						<p>
							...
							<span className="text-brown-light"> {config.groom.relation}</span>{' '}
							<span className="font-medium text-brown-dark">{config.groom.firstName}</span>
						</p>
						<p className="mt-1">
							{config.bride.fatherName} · {config.bride.motherName}
							<span className="text-brown-light"> {config.bride.relation}</span>{' '}
							<span className="font-medium text-brown-dark">{config.bride.firstName}</span>
						</p>
					</div>
				</div>
			</AnimateOnScroll>
