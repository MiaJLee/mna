...
export default function TransportSection({ config }: { config: WeddingConfig }) {
	return (
					...
					<AnimateOnScroll key={idx} delay={idx * 100}>
						<div className="bg-warm-white rounded-xl p-5 border border-beige/50">
							<div className="flex items-start gap-3">
								<span className="text-xl mt-0.5">{icons[item.type] || '📍'}</span>
								<div>
									<h3 className="text-sm font-medium text-brown-dark mb-2">{item.title}</h3>
									<ul className="space-y-1">
										{item.details.map((detail, i) => (
											<li key={i} className="text-xs text-brown leading-relaxed">
												{detail}
											</li>
										))}
									</ul>
